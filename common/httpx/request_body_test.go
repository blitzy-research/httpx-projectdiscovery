...
// These tests pin how a request BODY is framed on the wire and what happens to it across a
// redirect. Framing decides whether the origin sees the intended bytes at all, so a defect
// here corrupts every POST-based probe silently.
//
// Two layers are used, dictated by what has to be observed:
//
...
//     serializer and only exists once bytes have crossed a real socket, so a transport mock -
//     which intercepts ABOVE serialization - cannot express it. httptest binds 127.0.0.1, so
//     nothing is resolved and nothing egresses.
//   - The scripted mock transport for the REDIRECT cases, which need the per-hop request
//     stream with cloned headers and buffered bodies.
//
// redirect_test.go owns redirect POLICY, including the five-status method-rewriting sweep and
// the GetBody discriminator. This file owns the BODY-FRAMING consequence and asserts it for
// two representative statuses: one that preserves the method and must replay the payload, one
// that rewrites the method and must drop it.
//
// No test here uses t.Parallel(): New sets the process-global GODEBUG variable on the
// HTTP/1.1 path, which is unsafe to race, and the package is sequential by convention.

const (
	// requestBodyPayload is the payload for the loopback framing tests. Its length is
	// asserted as a precondition wherever it is used, so an edit to the literal cannot
	// silently invalidate the byte-count assertions built on it.
	...
	// requestBodyEchoContentType keeps the response body out of MIME sniffing so DecodeData
	// never transcodes it: the echoed bytes must come back byte-identical.
	requestBodyEchoContentType = "application/octet-stream"

	// The scripted two-hop chain. Both authorities are synthetic, live only inside the mock
	// transport's route map and are never resolved.
	...
	// so the FollowRedirects budget guard is never what ends the chain; the budget boundary
	// itself is owned by TestRedirectMaxRedirectsBudget.
	requestBodyMaxRedirects = 10
)

// opaqueBodyReader hides everything about its payload except how to read it: it exposes ONLY
// Read, with no Len, Size or Seek, so it misses every case
// readerutil.NewReusableReadCloser special-cases. A body whose length cannot be discovered
// without consuming it is the one body for which chunked framing would be the natural
// choice, which is what makes it the case that proves buffering happens.
...
// capturedRequest is the server's view of one inbound request, as a value type. It carries no
// mutex, so it can be returned and compared by value without tripping govet's copylocks
// check; requestBodyRecorder owns the lock. Its fields are the wire-framing facts an observer
// on the connection could see.
type capturedRequest struct {
	...
	// server-side Request.Header retains the Content-Length line, so this and ContentLength
	// are independent views of the same framing decision.
	ContentLengthHdr string
	// TransferEncoding is nil for a length-delimited message and []string{"chunked"} for a
	// chunked one, copied so the recording cannot alias server-owned state.
	TransferEncoding []string
	// Body holds every byte the handler read from r.Body.
	Body []byte
}

// requestBodyRecorder captures the server's view of the requests an echo server received. The
// mutex is required because the handler runs on a goroutine owned by the httptest server while
// the assertions run on the test goroutine; pointer receivers keep the lock from being copied.
type requestBodyRecorder struct {
	mu sync.Mutex
	// calls counts inbound requests, so a test can assert that exactly one reached the
	// server: a silent retry or unexpected redirect would otherwise leave the captured
	// framing describing a request the test never meant to inspect.
	calls int
	last  capturedRequest
	// readErr holds the first body-read failure, stored rather than asserted in place because
	// require's FailNow calls runtime.Goexit, which on a server goroutine would abandon the
	// response; result() re-raises it on the test goroutine where FailNow is safe.
	readErr error
}

// record drains the request body, stores the framing snapshot and returns the bytes read so
// the handler can echo them back. Draining r.Body fully is mandatory: a handler that never
// reads the body would capture zero bytes and every "exact bytes" assertion would pass
// vacuously.
...
// newRequestBodyEchoServer starts a loopback server that records the framing of each request
// and echoes the received body back verbatim. Echoing turns the response into an INDEPENDENT
// witness: Response.Data can only hold the payload if those exact bytes reached the origin and
// came back, a second proof that shares no mechanism with the server-side capture.
//
// The server is returned WITHOUT a registered cleanup, matching the repository's own
// newWellKnownTestServer helper, so every caller pairs it with a visible `defer ts.Close()`
// at the site that owns its lifetime.
...
//
// The 13 supplied bytes are compared against the 13 bytes the handler read, the declared
// length against that number twice (the parsed framing field and the raw header line), and
// the echoed response against the payload a third time on a path that shares no mechanism
// with the first two. Length-delimited framing for a body of known size is what RFC 9112
...
// TestRequestBodyBufferedRegardlessOfReaderType pins the counter-intuitive contract: for a
// payload of NON-ZERO length, a length-bearing reader and a reader that exposes NO length
// produce byte-for-byte IDENTICAL framing on the wire, and chunked framing never occurs for
// either. The table also pins the single input class that IS chunked, which keeps the "never
// chunked" rows a real discrimination.
//
// WHY: retryablehttp's constructor does not hand the caller's reader to net/http. It wraps
// ANY body in readerutil.NewReusableReadCloser, which buffers every input kind into a
// bytes.Buffer - the generic io.Reader case included - and then measures the length by
// draining that buffer. For a body of non-zero length an exact Content-Length is therefore
// available by the time net/http serializes the request, and no transfer coding is needed.
//
// THE ONE EXCEPTION, pinned by the last two rows: a body that is NOT nil but buffers to ZERO
// bytes is sent with Transfer-Encoding: chunked and no Content-Length line at all. Buffering
// alone therefore does not guarantee a declared length - a buffered body of non-zero length
// does. The mechanism is entirely inside the standard library:
//
//   - Request.outgoingLength cannot tell "length 0, known" apart from "length unknown" on a
//     non-nil Body, so it maps (Body != nil && ContentLength == 0) to -1, meaning unknown.
//   - newTransferWriter takes that -1 and consults shouldSendChunkedRequestBody, which
//     returns true WITHOUT probing the body for a POST, because requestMethodUsuallyLacksBody
//     covers only GET/HEAD/DELETE/OPTIONS/PROPFIND/SEARCH.
//
// RFC 9112 section 6.3 leaves no alternative once the length is unknown: a message that
// declares neither a length nor a transfer coding has no body at all, so chunked framing is
// the only correct way to send one. The behaviour is CORRECT rather than a defect, and it is
// pinned because a library caller can reach it - httpx's own scanning path always passes
// either an explicit request body or nil - and because a silent framing change here is the
// same class of defect as an unwanted chunked body anywhere else: some origins and WAFs
// reject chunked requests.
//
// The rows are consequently sharp: replacing the buffering with a streaming body would make
// the length of every non-empty row unknown, so net/http would fall back to chunked framing
// with ContentLength -1 and those rows would fail. The control sub-test shows that alternative
// is genuinely reachable at the same server, using the same opaque reader and the same 13
// bytes without the retry layer.
//
// The nil-body row is worth stating explicitly: net/http emits Content-Length: 0 rather than
// omitting the header, which RFC 9110 section 8.6 permits for a bodyless request with a method
// that defines body semantics. Read against the two rows below it, that is what shows the
// discriminator is nil-ness rather than size - a nil body takes the Body == nil branch of
// outgoingLength and is length-delimited, while a non-nil body of the same zero size is not.
func TestRequestBodyBufferedRegardlessOfReaderType(t *testing.T) {
	...
	cases := []struct {
		...
		// wantRequestBodyPresent is the discriminator the framing decision hinges on:
		// outgoingLength returns a definite 0 for a nil Body and -1 (unknown) for a non-nil
		// Body of length 0, which is why the nil row is length-delimited and the empty
		// non-nil rows are chunked.
		...
		// wantContentLength is the length the CONSTRUCTOR computes before serialization.
		// Because the server echoes the body back verbatim it is also the caller-visible
		// response length, so it is asserted at both ends.
		wantContentLength int64
		// wantOriginContentLength is the framing value net/http PARSED at the origin: the
		// declared length, or -1 for a chunk-delimited message. It differs from
		// wantContentLength only for a body that is non-nil yet empty.
		...
		},
		{
			...
			// input class this client chunks, and an unnoticed change here would be as
			// harmful as an unwanted chunked body on any other path.
		...
		{
			// The same zero-length payload behind a type that exposes no length: identical
			// framing, which shows the divergence is driven by the buffered SIZE rather than
			// by the reader's concrete type.
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			require.Equal(t, tc.wantRequestBodyPresent, req.Body != nil,
				"whether a body object exists at all is what net/http reads a zero length as: outgoingLength gives a definite 0 for a nil Body and an unknown length for a non-nil one")
	...
	t.Run("control: the same opaque reader streamed without the retry layer frames as chunked", func(t *testing.T) {
		// This control asserts net/http's fallback, not this client's behaviour: it proves the
		// length-delimited rows above distinguish two genuinely reachable states at this very
		// server. Chunked framing alone does NOT imply the retry layer was bypassed, because
		// the retry layer also chunks a non-nil body of zero length; what identifies a
		// streaming regression is chunked framing for a body of NON-ZERO length, which is what
		// this control reproduces with the same 13 bytes. Its transport is dedicated and its
		// pool is closed afterwards, so no global state is touched.
		rec := &requestBodyRecorder{}
		ts := newRequestBodyEchoServer(t, rec)
		defer ts.Close()
...
//
// recordedRequest stores the method, URL, cloned header and buffered body but NOT the framing
// fields, because Content-Length is not a header on a client-side request - net/http
// serializes it from Request.ContentLength. Capturing these separately in the handler is
// therefore the only way to assert per-hop framing.
...
//
// Every call builds its OWN transport and client, so the two callers cannot observe each
// other's hop counts or option values. The framing slice is appended from the handler, which
// net/http invokes synchronously on the goroutine that called Do, so no lock is needed here -
// unlike the loopback echo handler, which runs on a server goroutine and uses a mutex.
func runRequestBodyRedirect(t *testing.T, firstHopStatus int) (*Response, []recordedRequest, []requestFraming) {
	...
	// CheckRedirect closure it builds, so newMockHTTPX runs this mutator before New. It also
	// installs the transport on both HTTP clients, which is what keeps every path off the
	// network.
...
// body are NOT altered: both hops leave as POST carrying the same 7 bytes with the same
// declared length.
//
// The per-hop view is trustworthy only because the harness snapshots the body and clones the
// header BEFORE delegating: net/http mutates and reuses request objects while following a
// redirect, so a mock that stored pointers would report the final state for every hop and
...
func TestRequestBodyReplayedOn307(t *testing.T) {
	...
	// Content-Length is absent from the client-side header map on BOTH hops: net/http
	// serializes it from Request.ContentLength and never writes it into Request.Header, which
	// is why the framing assertions above read the field instead.
...
// net/http does exactly that. Asserting the drop matters as much as asserting the replay: a
// defect that carried the payload onto a rewritten GET would send a body the origin never
// expects on a method with no body semantics.
//
// The five-status method-rewriting sweep lives in TestRedirectMethodAndBodyRewriting; this
// test asserts the BODY-FRAMING consequence for the representative rewriting status only.
func TestRequestBodyDroppedOn302(t *testing.T) {
	...
	// The dropped body is not announced as "Content-Length: 0" in the client-side header map
	// either: net/http keeps the length in Request.ContentLength, which the framing assertion
	// above reads as exactly 0.
	require.Equal(t, "", hops[0].Header.Get("Content-Length"),
		"a client-side request carries its length in Request.ContentLength, not in the header map")
	require.Equal(t, "", hops[1].Header.Get("Content-Length"),
